...
const ManagerRoute = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      console.log('ManagerRoute: Checking role', { currentRole, apiRole: role, storedRole, loading, hasUser: !!user });
      
      // If not a manager, redirect to appropriate route
      if (currentRole && currentRole !== 'Manager') {
        ...
        console.log('ManagerRoute: Redirecting non-manager user', { 
  ...
  if (loading || !user) {
    console.log('ManagerRoute: Showing spinner while loading or waiting for user data');
    return <div className="h-screen flex items-center justify-center"><Spinner /></div>;
  }

  // If we have role info and it's not manager, show spinner while redirecting
  const currentRole = role || storedRole;
  if (currentRole && currentRole !== 'Manager') {
    console.log('ManagerRoute: Showing spinner while redirecting non-manager user');
    return <div className="h-screen flex items-center justify-center"><Spinner /></div>;
  }

  // Only render children if we have both role and user data
  if (!user || !currentRole) {
    console.log('ManagerRoute: Showing spinner while waiting for complete data');
    return <div className="h-screen flex items-center justify-center"><Spinner /></div>;
  }

  console.log('ManagerRoute: Rendering children, all checks passed');
  return children;
};
