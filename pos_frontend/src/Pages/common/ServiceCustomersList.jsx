...
import { createStatusFilter, createSelectFilter } from "../../utils/dynamicFilterUtils";
...
function ServiceCustomersList() {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      if (popupName === "Edit User" && currentUser) {
        ...
        response = await createUser({ ...formData, role: "Service Customer" });
  ...
  const cells = [
    ({ row }) => (
      <div className="flex items-center cursor-pointer" onClick={() => navigate(`service-customer/${row.id}`)}>
  ...
  return (
    <>
      ...
      <TableComponent
        ...
        cells={cells}
        data={customers}
        heading="Service Customers"
        description="Create and manage Service Customers here"
        createBtn={true}
        ...
        pageId={PAGE_IDS.SERVICE_CUSTOMER_LIST}
        itemsPerPage={10}
      ...
      {(popupName === "Create User" || popupName === "Edit User") && (
        <PopupComponent popup={popup} setPopup={setPopup} loading={loading}>
          <UserFormPopup
            ...
            roleFixed={popupName === "Create User" ? { roles: ["Service Customer"] } : { roles: ["Service Customer", "Vending Customer"] }}
...
export default ServiceCustomersList; 